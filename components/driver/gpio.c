...
esp_err_t gpio_config(const gpio_config_t *pGPIOConfig)
{
    ...
    do {
        ...
        if (((gpio_pin_mask >> io_num) & BIT(0))) {
            ...
            //FIXME: how to avoid RFID(HTRC110 needs to config a pin as IN/OUT frequently) issueing so many messages herein???
            // KKK ESP_LOGI(GPIO_TAG, "GPIO[%d]| InputEn: %d| OutputEn: %d| OpenDrain: %d| Pullup: %d| Pulldown: %d| Intr:%d ", io_num, input_en, output_en, od_en, pu_en, pd_en, pGPIOConfig->intr_type);
